...
if user_message:
    ...
    response = requests.post(
        "https://mayank46.app.n8n.cloud/webhook/a9d1b658-4439-4d3f-ab78-0441cb1eaa40",  # replace with your n8n webhook URL
    ...
    try:
        if response.status_code != 200:
            ai_response = f"⚠️ Server error ({response.status_code}). Try again."
        
        elif not response.text.strip():
            ai_response = "⚠️ Empty response from assistant. Try again."
        
        else:
            data = response.json()

            if isinstance(data, list) and len(data) > 0:
                ai_response = data[0].get("output", "⚠️ No output returned.")
            elif isinstance(data, dict):
                ai_response = data.get("output", "⚠️ No output returned.")
            else:
                ai_response = "⚠️ Unexpected response format."

    except Exception:
        ai_response = "⚠️ Assistant failed to respond properly. Try again."
